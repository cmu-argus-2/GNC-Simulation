...
pose_simulator_build_script_abs = os.path.join(
    repo_root_abs, pose_simulator_build_script_rel
)
parameter_file_abs = os.path.join(
    repo_root_abs, montecarlo_rel, "configs/", parameter_file_name
)
results_directory_abs = os.path.join(repo_root_abs, montecarlo_rel, "results/")
plot_script_abs = os.path.join(
    repo_root_abs, montecarlo_rel, "plots_and_analysis/plot.py"
)
trial_executable_abs = os.path.join(repo_root_abs, trial_executable_path_rel)
...
), f"Nonexistent: {pose_simulator_build_script_abs}"
...
def record_git_state(job_directory_abs):  # generated by ChatGPT
    ...
    diff = subprocess.check_output(
        ["git", "diff"], cwd=repo_root_abs, universal_newlines=True
    )
...
def plot_results(job_name):
    ...
    process = subprocess.Popen(
        ["python3", plot_script_abs, job_name], cwd=os.path.dirname(plot_script_abs)
    )
