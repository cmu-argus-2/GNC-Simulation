...
/* Interesting Initial Parameters:
	...
	x_init = 0 y_init = 0 theta_init = 47 vel_init = 11.08136e3 dt = 10 //leave earth, go extremely close to the moon, then come back close to earth
*/
...
Vector4d f(VectorXd x, double u) {
	Vector2d earth_pos; earth_pos << x_earth, y_earth;
	Vector2d moon_pos; moon_pos << x_moon, y_moon;
	Vector2d sat_pos; sat_pos << x(0), x(2);
	...
	Vector2d a = G * (((m_earth / pos_sat_to_earth.squaredNorm()) * pos_sat_to_earth.normalized()) +
	                  ((m_moon / pos_sat_to_moon.squaredNorm()) * pos_sat_to_moon.normalized())); //satellite's acceleration vector due to gravity [m/s^2]
...
void plotCircle(double x_center, double y_center, double radius, char *label){
	std::vector<double> x;
	std::vector<double> y;
